            ...
            <div>
                ...
                <main class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                    <div class="flex items-baseline justify-between border-b border-gray-200 pb-6 pt-24">
                        <h1 class="text-4xl font-bold tracking-tight text-gray-900">Store</h1>
                    ...
                    <section aria-labelledby="products-heading" class="pb-24 pt-6">
                        ...
                        <div class="grid grid-cols-1 gap-x-8 gap-y-10 lg:grid-cols-4">
                                ...
                                <div class="flex items-center justify-between space-x-3">
                                    <div class="basis-1/3">
                                        <label for="filters-price-from"
                                               class="mb-2 block text-sm font-medium text-gray-900 dark:text-white">From</label>
                                        <input type="number" id="filters-price-from" min="0"
                                               :max="filterPricesForm.prices[1]"
                                               placeholder="min-price" v-model="filterPricesForm.prices[0]"
                                               class="block w-full border-gray-300 border bg-gray-50 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                                        />
                                    </div>
                                    <div class="basis-1/3">
                                        <label for="filters-price-to"
                                               class="mb-2 block text-sm font-medium text-gray-900 dark:text-white">To</label>
                                        <input type="number" id="filters-price-to" :min="filterPricesForm.prices[0]"
                                               placeholder="max-price" v-model="filterPricesForm.prices[1]"
                                               class="block w-full border-gray-300 border bg-gray-50 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"/>
                                    </div>
                                    ...
                                    <DisclosurePanel class="pt-6">
                                        <div class="space-y-4">
                                            <div v-for="brand in brands" :key="brand.id"
                                                ...
                                                <input :id="`filter-brand-${brand.id}`" v-model="selectedBrands"
                            ...
                            <div class="lg:col-span-3">
                                <!-- Your content -->
                                <Products :products="products.data" v-if="products.data.length>0"></Products>
                                <div v-else class="flex justify-center items-center h-96">
                                    <h1 class="text-3xl font-bold">No products found</h1>
                                </div>
...
const sortOptions = [
    { name: 'Newest',  current: true, slug:'newest'  },
    { name: 'Most Popular', current: false, slug: 'most-popular' },
    { name: 'Price: Low to High',  current: false, slug: 'price-low-to-high' },
    { name: 'Price: High to Low',  current: false, slug: 'price-high-to-low' },
]
...
// const priceFilter = () => {
//     filterPricesForm.transform((data) => ({
//         ...data,
//         prices: {
//             from: filterPricesForm.prices[0],
//             to: filterPricesForm.prices[1]
//         }
//     })).get('products', {
//         preserveState: true,
//         replace: true,
...
const validatePrices = () => {
    if (filterPricesForm.prices[0] > filterPricesForm.prices[1]) {
        Swal.fire({
            icon: 'error',
            title: 'Oops...',
            text: 'The minimum price cannot be greater than the maximum price!',
        })
        return false
    }
    if (filterPricesForm.prices[0] < 0 || filterPricesForm.prices[1] < 0) {
        Swal.fire({
            icon: 'error',
            title: 'Oops...',
            text: 'The price cannot be negative!',
        })
        return false
    }
    return true
...
const updateFilteredProducts = async () => {
    ...
    try {
        await router.get('products', data, {
            preserveState: true,
            replace: true,
        })
    } catch (e) {
...
const clearFilters = () => {
    ...
    filterPricesForm.prices = [0, 100000]
    setSortOption(sortOptions[0])
    usePage().props.search = null
    updateFilteredProducts()
}
</script>
