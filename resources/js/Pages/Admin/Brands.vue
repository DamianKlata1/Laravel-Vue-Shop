    ...
    <AdminLayout>
        <section class="bg-gray-50 dark:bg-gray-900 p-3 sm:p-5">
            <el-dialog
                ...
                :title="isEditBrand ? 'Edit Brand' : 'Add Brand'"
                width="30%"
                :before-close="handleDialogClose"
            >
                <form @submit.prevent="isEditBrand ? editBrand() : addBrand()" class="max-w-md mx-auto">
...
const editBrand = async () => {
    ...
    try {
        await router.post(`/admin/brands/update/${id.value}`, formData, {
            onSuccess: page => {
                Swal.fire({
                    toast: true,
                    icon: "success",
...
const deleteBrand = async(brand) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            try {
                await router.delete(`/admin/brands/delete/${brand.id}`, {
                    onSuccess: page => {
                        Swal.fire({
                            toast: true,
                            icon: "success",
