<template>
    <AdminLayout>
        <section class="bg-gray-50 dark:bg-gray-900 p-3 sm:p-5">
            ...
            >
                <form @submit.prevent="isEditCategory? editCategory() : addCategory()" class="max-w-md mx-auto">
                        ...
                        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            ...
                            <tr v-for="category in categories" :key="category.id" class="border-b dark:border-gray-700">
                                ...
                                <td class="px-4 py-3 flex items-center justify-end">
                                    <button :id="`${category.id}-dropdown-button`" :data-dropdown-toggle="`${category.id}-dropdown`"
                                    ...
                                    <div :id="`${category.id}-dropdown`"
                                        ...
                                        <div class="py-1">
                                            <button @click="deleteBrand(category)"
...
const deleteBrand = async(category) => {
    Swal.fire({
        title: 'Are you sure?',
        ...
        if (result.isConfirmed) {
            try {
                await router.delete(`/admin/categories/delete/${category.id}`, {
                    onSuccess: page => {
                        Swal.fire({
                            toast: true,
                            icon: "success",
